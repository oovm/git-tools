...
Spread commit **author** and **committer** timestamps across a date range by rewriting commit objects and pointing a new
...
Trees, blobs, and commit messages are reused (except root `--message`). Only author/committer timestamps change (plus
parent relinking when ancestors are rewritten).
