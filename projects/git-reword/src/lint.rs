...
pub fn lint_message(raw: &str, label: &str) -> Vec<LintIssue> {
    ...
    let body_joined = if lines.len() > 1 {
        lines[1..].join("\n")
    } else {
        String::new()
    };
    ...
    if !emoji.is_match(subject) {
        issues.push(LintIssue {
            label: label.to_string(),
            detail: "subject must start with a gitmoji character".into(),
        });
    }
    ...
    if full.contains(';') || full.contains('；') {
        issues.push(LintIssue {
            label: label.to_string(),
            detail: "must not contain semicolons".into(),
        });
    }
    let milestone =
        Regex::new(r"(?i)\b(phase\s*[-_]?\s*\d|m\d|s\d|a\d|f\d|gate[- ]?\d)\b").expect("milestone");
    if milestone.is_match(&full) {
        issues.push(LintIssue {
            label: label.to_string(),
            detail: "must not contain internal milestone codes".into(),
        });
    }
    ...
    if bare_ts.is_match(&full) {
        issues.push(LintIssue {
            label: label.to_string(),
            detail: "write TypeScript in full instead of bare TS".into(),
        });
    }
    ...
    if version.is_match(&full) {
        issues.push(LintIssue {
            label: label.to_string(),
            detail: "avoid version numbers in commit messages".into(),
        });
    }
    issues
}
